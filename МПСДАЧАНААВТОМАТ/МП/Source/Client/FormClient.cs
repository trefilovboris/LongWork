...
namespace Client
{
    public partial class ClientForm : Form, ICommandCallback
    {
        void ICommandCallback.Callback(string callbackResult, byte[] data, int codeCommand)
        {
            /* if (codeCommand == (int)Comands.Upload && callbackResult != "Нету такого файла!\n" && callbackResult != "Заполните все предложенные поля\n" && callbackResult != "Нет прав!\n")
             {
                  ResultTextBox.Text = new FunctionsClient().WriteFile(Argument2.Text + '*' + callbackResult, data);
             }
             else
             {
                 ResultTextBox.Text = callbackResult;
             }*/
        }
